<?php
if(!(empty($_POST["f-name"]) && empty($_POST["l-name"])&& empty($_POST["email"])&& empty($_POST["mobile"])&& empty($_POST["password"]))) {
    require "config.php";
    $fname = $_POST["f-name"];
    $lname = $_POST["l-name"];
    $email = $_POST["email"];
    ...
    echo $password;
    $sql = "INSERT INTO `login` (fname, lname, email, mobile, password)
    VALUES ('$fname', '$lname', '$email', '$mobile', '$password')";
    if ($conn->query($sql) === TRUE) {
        $txt = '<p>Your account is created successfully, please login and continue shopping</p>';
    } 
    else {
